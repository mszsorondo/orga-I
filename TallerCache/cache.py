from politicas import FIFO, MRU, LRU, RANDOM
...
class CacheAsociativa_NWays(object):
    def __init__(self,
        ...
        self.ways = [[{
            ...
            'stepChange': -1,
            'stepFirstUse': -1
        } for ii in range(self.nSets)] for i in range(self.nWays)]
        self.log = []
        self.step = -1
        ...
        self.nbits_tag = int(
            ceil(
                log(
                    ceil(float(self.memorySize) / (self.sizeSet * self.nSets)),
                    2)))
    ...
    def getFields(self, address):
        ...
        return 'tag: ' + str(f['tag']) + ' set: ' + str(
            f['set']) + ' index: ' + str(f['index'])

    def _getFields(self, address):
        # dada una direccion obtengo a que tag, set e indice corresponde
        assert (
            self.memorySize > address
        )
        # , "address out of range:" + address + ". Memory size:" +
        # self.memorySize
        adress_base2 = ("".join(["0" for i in range(self.nbits_total)]) + str(
            bin(address)[2:]))[-self.nbits_total:]

        d_tag = int(adress_base2[:self.nbits_tag], 2)
        try:
            d_set = int(
                adress_base2[self.nbits_tag:self.nbits_tag + self.nbits_set],
                2)
        except:
            d_set = 0  # TODO: chequear q este bien esto
    ...
    def _fields2Memory(self, tag, set, index):
        binary = ("".join(["0" for i in range(self.nbits_total)]) +
                  "{0:b}".format(tag))[-self.nbits_tag:]
        binary += ("".join(["0" for i in range(self.nbits_total)]) +
                   "{0:b}".format(set))[-self.nbits_set:]
        binary += ("".join(["0" for i in range(self.nbits_total)]) +
                   "{0:b}".format(index))[-self.nbits_index:]
        return int(binary, 2)
    ...
    def fetch(self, address):
        ...
        # Tengo que revisar si esta guardado en la cache, para eso recorro cada
        # via
        for wi, way in enumerate(self.ways):
            ...
            if d['tag'] == values_per_field['tag']:  # Hit!
                ...
                dato = self.ways[wi][values_per_field['set']]['mem'][
                    values_per_field['index']]
        ...
        way2Use = self.cacheAlg(self.ways, values_per_field['tag'],
                                values_per_field['set'])
        self.ways[way2Use][values_per_field['set']]['stepFirstUse'] = newStep
        self.ways[way2Use][values_per_field['set']]['stepChange'] = newStep
        self.ways[way2Use][values_per_field['set']]['tag'] = values_per_field[
            'tag']
        self.ways[way2Use][values_per_field['set']]['address_req'] = address
        self.ways[way2Use][values_per_field['set']]['address'] = [
            int(self.sizeSet * int(address / self.sizeSet)) + i
            for i in range(self.sizeSet)
        ]
        self.ways[way2Use][values_per_field['set']]['mem'] = [
            self.memory[a]
            for a in self.ways[way2Use][values_per_field['set']]['address']
        ]
        self.ways[way2Use][values_per_field['set']]['valid'] = True
        self.appenToLog(address, False, way2Use, newStep)

        # Obtengo el dato de la linea de cache
        dato = self.ways[way2Use][values_per_field['set']]['mem'][
            values_per_field['index']]
    ...
    def fetchFrom(self, fileName):
        ...
        file = open(fileName)
        access_list = np.array(file.read().split("\n"))  # parse file
        file.close()
        access_list = access_list[access_list != '']  # remove empty elems
        access_list = [int(addr, 16)
                       for addr in access_list]  # convert from str to hex
    ...
    def info(self):
        ...
        res += 'Algoritmo de sustitucion: ' + str(
            self.cacheAlg.__name__) + "\n"
        res += "\n"
        print(res)
    ...
    def mostrarLog(self):
        for c in self.log:
            print('|	Step: %d	|	%s	|	Linea: %d	|	Direccion: %d	|' %
                  (c['step'], 'Hit'
                   if c['hit'] else 'Miss', c['line'], c['address']))


class CacheTotalmenteAsociativa(CacheAsociativa_NWays):
    def __init__(self, memory, cacheSize=8, nLines=2, cacheAlg=FIFO):
        super(CacheTotalmenteAsociativa, self).__init__(
            memory,
            cacheSize=cacheSize,
            nWays=nLines,
            nSets=1,
    ...
    def get_to_plot(self, line):
        return super(CacheTotalmenteAsociativa, self).get_to_plot(
            way=line, set=0)
    ...
    def infoCache(self, line):
        print('Linea %d:' % (line))
        return super(CacheTotalmenteAsociativa, self).infoCache(
            way=line, set=0)


class CacheCorrespondenciaDirecta(CacheAsociativa_NWays):
    def __init__(self, memory, cacheSize=8, nLines=2):
        super(CacheCorrespondenciaDirecta, self).__init__(
            memory, cacheSize=cacheSize, nWays=1, nSets=nLines, cacheAlg=FIFO)

    def get_to_plot(self, line):
        return super(CacheCorrespondenciaDirecta, self).get_to_plot(
            way=0, set=line)
    ...
    def infoCache(self, line):
        ...
        return super(CacheCorrespondenciaDirecta, self).infoCache(
            way=0, set=line)
